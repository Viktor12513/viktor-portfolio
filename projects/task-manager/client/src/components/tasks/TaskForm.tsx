...
export function TaskForm({ onSubmit, editingTask, onCancelEdit, isSubmitting }: TaskFormProps) {
  ...
  const validate = () => {
    ...
    if (!values.dueDate) {
      nextErrors.dueDate = 'Choose a due date so the task has a clear deadline.';
    }
    ...
    <section className="panel p-6">
      <div className="mb-6">
        ...
        <h2 className="mt-3 text-2xl font-semibold text-ink dark:text-white">
          {editingTask ? 'Update an existing task' : 'Create a new task'}
        </h2>
        <p className="mt-2 text-sm leading-7 text-slate-600 dark:text-slate-300">
          Keep tasks clear and short so they are easy to scan and manage.
        </p>
      </div>
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">
          ...
          <textarea
            ...
            placeholder="Add loading and error states to the task list."
            className="mt-2 w-full rounded-2xl border bg-white px-4 py-3 text-sm outline-none transition focus:border-slateblue dark:bg-slate-900"
          />
          {errors.description ? (
