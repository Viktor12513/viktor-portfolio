...
type ExpenseDemoListProps = {
  entries: ExpenseEntry[];
  busyEntryId: number | null;
  onDelete: (entryId: number) => Promise<void>;
};

export function ExpenseDemoList({ entries, busyEntryId, onDelete }: ExpenseDemoListProps) {
  if (entries.length === 0) {
    return (
      <section className="panel p-8 text-center">
        <h4 className="text-2xl font-semibold text-ink dark:text-white">No entries yet</h4>
        <p className="mt-3 text-sm leading-7 text-slate-600 dark:text-slate-300">
          Add your first income or expense entry to try the demo.
        </p>
      </section>
    );
  }
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h4 className="text-lg font-semibold text-ink dark:text-white">{entry.title}</h4>
                <span
              ...
              <div className="mt-3 flex flex-wrap gap-4 text-sm text-slate-500 dark:text-slate-400">
                ...
                <span className={entry.type === 'income' ? 'font-semibold text-emerald-600' : 'font-semibold text-rose-600'}>
